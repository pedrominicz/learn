...
-/
example {A : set ℝ} {x : ℝ} (hx : x is_an_inf_of A) :
  ∀ y, x < y → ∃ a ∈ A, a < y :=
begin
  intro y,
  contrapose,
...
begin
  contrapose!,
  from λ h, ⟨(y-x)/2, by linarith, by linarith⟩,
end
...
very quickly
-/
example {x y : ℝ} {u : ℕ → ℝ} (hu : limit u x) (ineq : ∀ n, y ≤ u n) : y ≤ x :=
begin
  ...
  calc  
    y ≤ u N           : ineq N
  ... = x + (u N - x) : by linarith
  ... ≤ x + |u N - x| : add_le_add (by refl) (le_abs_self _)
  ... ≤ x + ε         : add_le_add (by refl) (HN N (by unfold ge))
end
