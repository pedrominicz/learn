...
example (f g : ℝ → ℝ) (hf : non_decreasing f) (hg : non_decreasing g) :
  non_decreasing (g ∘ f) :=
assume x₁ x₂ h,
have f x₁ ≤ f x₂, from hf _ _ h,
show g (f x₁) ≤ g (f x₂), from hg _ _ this
...
begin
  ...
  have : f x ≤ x ∨ x ≤ f x, from le_total _ _,
